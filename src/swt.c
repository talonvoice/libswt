...
void ccv_swt(ccv_dense_matrix_t* a, ccv_dense_matrix_t** b, int type, ccv_swt_param_t params) {
	...
	ccv_array_t* strokes = ccv_array_new(sizeof(ccv_swt_stroke_t), 64);
...
static ccv_array_t* _ccv_swt_connected_component(ccv_dense_matrix_t* a, int ratio, int min_height, int max_height, int min_area)
{
	...
	ccv_array_t* contours = ccv_array_new(sizeof(ccv_contour_t*), 5);
	for (i = 0; i < a->rows; i++)
	{
		for (j = 0; j < a->cols; j++)
...
static ccv_array_t* _ccv_swt_connected_letters(ccv_dense_matrix_t* a, ccv_dense_matrix_t* swt, ccv_swt_param_t params)
{
	...
	ccv_array_t* letters = ccv_array_new(sizeof(ccv_letter_t), 5);
	int i, j, x, y;
	...
	ccv_array_t* new_letters = ccv_array_new(sizeof(ccv_letter_t), 5);
...
{
	int i, j;
	ccv_array_t* pairs = ccv_array_new(sizeof(ccv_letter_pair_t), letters->rnum);
	...
	ccv_array_t* regions = ccv_array_new(sizeof(ccv_textline_t), 5);
	for (i = 0; i < nchains; i++)
...
static ccv_array_t* _ccv_swt_break_words(ccv_array_t* textline, ccv_swt_param_t params)
{
	...
	ccv_array_t* words = ccv_array_new(sizeof(ccv_rect_t), textline->rnum);
	for (i = 0; i < textline->rnum; i++)
	{
		ccv_textline_t* t = (ccv_textline_t*)ccv_array_get(textline, i);
		...
		ccv_dense_matrix_t otsu = ccv_dense_matrix(1, t->neighbors - 1, CCV_32S | CCV_C1, buffer);
		double var;
...
ccv_array_t* ccv_swt_detect_words(ccv_dense_matrix_t* a, ccv_swt_param_t params) {
    ...
    {
        textline2 = ccv_array_new(sizeof(ccv_textline_t), ntl);
        ...
        ccv_array_free(lettersB);
        ccv_array_free(lettersF);
        words = ccv_array_new(sizeof(ccv_rect_t), ntl);
